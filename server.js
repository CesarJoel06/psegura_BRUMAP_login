...
// -----------------------------------------------------------------------------
// Helpers de formato de fecha originales
// -----------------------------------------------------------------------------
...
app.use(helmet({
  contentSecurityPolicy: {
    ...
    directives: {
      "default-src": ["'self'"],
      "script-src": ["'self'"],        // sin inline: front debe usar solo JS externo
      "img-src": ["'self'", "data:"],
      "style-src": ["'self'"],
      "object-src": ["'none'"],
  ...
  // Nota: se puede extender con hsts, noSniff, etc., que Helmet ya maneja por defecto
}));

// Compresión – mejora rendimiento, no afecta seguridad directamente
app.use(compression());
...
app.use(session({
  name: 'brumap.sid', // nombre de cookie no estándar (no 'connect.sid')
  secret: SESSION_SECRET || 'dev_secret_change_me', // para producción usar siempre env
  resave: false,
  saveUninitialized: false,
  cookie: {
    httpOnly: true,       // protege frente a XSS (no accesible por JS)
    sameSite: 'lax',      // evita la mayoría de CSRF básicos
    secure: COOKIE_SECURE, // sólo por HTTPS cuando COOKIE_SECURE=1
...
function csrfProtect(req, res, next) {
  ...
  if (['POST', 'PUT', 'PATCH', 'DELETE'].includes(m)) {
    // Login se deja fuera porque es la entrada al sistema
...
// -----------------------------------------------------------------------------
// Static – OWASP A05: cache correcto para HTML vs assets
// -----------------------------------------------------------------------------
const PUBLIC_DIR = path.join(__dirname, 'public');
// ruta al logo para PDFs
...
// -----------------------------------------------------------------------------
// Recepciones – OWASP A03 (SQL con parámetros), A01 (requireRole)
// -----------------------------------------------------------------------------
...
// -----------------------------------------------------------------------------
// PDFs BONITOS (logo + tablas) – datos solo de la BD, sin HTML del usuario
// -----------------------------------------------------------------------------

// Versión basada en Date para PDFs, pero usando helpers si falla
...
// Encabezado de PDF con logo, título y fecha
...
// Dibuja tabla con encabezado naranja y filas (texto negro)
...
// ---- PDF Registros completos (todas las tablas) ----
...
// -----------------------------------------------------------------------------
// Users – gestión de roles y contraseñas (A01, A07)
// -----------------------------------------------------------------------------
...
// -----------------------------------------------------------------------------
// Manejador global de errores – OWASP A05: no exponer stack trace al cliente
// -----------------------------------------------------------------------------
